        ...
        <div v-else class="max-w-7xl mx-auto space-y-8">
            ...
            <TransitionGroup name="list" tag="div" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 relative">
                <div v-if="filteredStats.length === 0" class="col-span-full text-center py-12">
                    <p class="text-text-muted text-xl">No statistics match these filters.</p>
                </div>
                <div v-for="stat in filteredStats" :key="stat.gameMode"
                    ...
                    <div class="items-start mb-6">
                        ...
                        <div class="flex gap-2">
                            <span class="text-xs text-text-muted font-semibold bg-subcard-background px-2 py-0.5 rounded-md">{{ stat.tmGame }}</span>
                            <span class="text-xs text-text-muted font-semibold bg-subcard-background px-2 py-0.5 rounded-md">{{ stat.tmCategory }}</span>
                        </div>
                    </div>
