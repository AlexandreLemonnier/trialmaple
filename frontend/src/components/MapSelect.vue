<template>
    <div ref="dropdownRef" class="relative w-full flex-1 min-w-0" @keydown="onKeyDown">
        <button class="flex w-full justify-between items-center text-lg lg:text-xl xl:text-2xl rounded-full border-2 border-app-border py-2 px-4 cursor-pointer"
                type="button"
                @click="isOpen = !isOpen">
        ...
        <ul v-if="isOpen"
            class="absolute text-md lg:text-lg xl:text-xl mt-2 w-full max-h-60 overflow-y-auto rounded-xl border border-app-border shadow-lg bg-app-background scrollbar-hide z-20">
